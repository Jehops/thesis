...
Shown in Appendix I is that for $C>0$ the large sample distribution of (\ref{eq:modlrs}) under the null hypothesis is $\chi_0^2/2+\chi_1^2/2$.
...
The results in table \ref{tab:sim} are over all sites in all simulated datasets.  After applying \gls{lr} tests at the 0.05 level to filter datasets that convey no evidence of positive selection, none of the false positive rates under \gls{beb} or \gls{sba} changed.  Supplementary table S1 gives the false positive rates under \gls{neb} after the adjustment.  With the exception of two cases, the effect is minimal.  Interestingly, under the null hypothesis, the false positive rates of the \gls{lr} tests were larger than expected, particularly with model misspecification.
