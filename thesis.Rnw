...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{.49\textwidth}
    ...
    edgelabels(c(expression('t'[6]),expression('t'[5]),expression('t'[1]),expression('t'[2]),expression('t'[3]),expression('t'[4])),adj=c(0,1),bg='white',cex=2,col='black',frame="none")
    @
    \caption{Rooted tree}
  ...
  \begin{subfigure}[t]{.49\textwidth}
    ...
    edgelabels(c(expression('','t'[5]),expression('t'[1]),expression('t'[2]),expression('t'[3]),expression('t'[4])),adj=2,bg='white',cex=2,col='black',frame="none")
    @
    \caption{Unrooted tree}
    \label{sfig:sampletreeunrooted}
  \end{subfigure}
  \caption[]{A four-taxon, rooted tree and the corresponding unrooted tree.  The rooted tree has 6 branches, 3 internal nodes (labelled 5, 6, and Root) and 4 external nodes (labelled 1, 2, 3, and 4).  The unrooted tree has 5 branches, 2 internal nodes (labelled 5 and 6), and the same 4 external nodes as the rooted tree.  The branch lengths are labelled $t_1$ through $t_6$.  The first two codons of the sequence data for the taxa at the external nodes is shown adjacent to the taxon label.}
...
\end{equation}
The observed site pattern at site $2$ shown in the sequence alignment in table \ref{tab:sampalign} is TTA, TTA, CTG, and TTA.  Calculation of the site likelihood using either tree in figure \ref{fig:sampletrees} will give the same result, because the model is time-reversible.  In the unrooted tree, the internal nodes are labelled $5$, and $6$ and the branch lengths are labelled $t_1$ through $t_5$.  To calculate the likelihood of the data at this site with parameters values $\theta$ given, one must condition on all possible unknown ancestral states
\begin{equation}
  \label{eq:sl}
  f(\bm{x}_h|\theta) = \sum_{x_5}\sum_{x_6}\left[\pi_{x_5}p_{x_5,TTA}(t_1)p_{x_5,TTA}(t_2)p_{x_5,x_6}(t_5)p_{x_6,TTA}(t_4)p_{x_6,CTA}(t_3)\right].
\end{equation}
% \begin{align}
%   \label{eq:sl}
%     \begin{array}{ll}
...
%     \end{array}
% \end{align}
Here $x_i$ represents the ancestral state and the $h$ subscript is omitted.  Using equations \ref{eq:PtEig} and \ref{eq:cm} along with the universal genetic code in table \ref{tab:code}, the probability of transitioning from, e.g., GTA at node $6$ to CTA at node $3$ over branch length $t_3$ is $\pi_{GTA}p_{GTA,CTA}(t_3) = \pi_{GTA}e^{\omega \pi_{CTA}t_3}$.  The quantity in the exponent is not scaled by $\kappa$ because G is a purine and C is a pyrimidine, but is scaled by $\omega$ because GTA and CTA do not code for the same amino acid, i.e., this codon substitution is a non-synonymous transversion.

Algorithms designed to maximize equation \ref{eq:ll} with respect to the parameters $\theta$ in order to obtain the ML parameter estimates, $\hat{\theta} = (\hat{\kappa},\hat{\omega},\hat{t_i})$ explore parameter space by adjusting the parameter values and repeat the likelihood calculation.  Each such iteration requires recalculation of transition probabilities over the tree and this must be done over all codon sites in the gene.  In the four-taxon tree like in figure \ref{fig:cmexampletree}, there are $61^2$ unobserved codon states to sum over.  Increasing the number taxa quickly makes likelihood maximization a computationally expensive task.

To economize on the computation of the likelihood over phylogenetic trees, the pruning algorithm was developed \citep{felsenstein1973maximum}, an application of Horner's rule, which eliminates redundant calculations. {\color{red} Need a few sentences to explain how the puring algorithm works}.
...
{\color{red} Fix this up for a SBA summary}
