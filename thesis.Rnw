...
\begin{align*}
  P_t &= P_{t\frac{n-1}{n}}P_{t\frac{1}{n}} \\
      ...
      &= \left(P_{t/n}\right)^n.
\end{align*}
When $n$ is large, the time interval $t/n$ is small.  From equation \ref{eq:Ph}, the transition probabilities in this short interval are $P(t) = [I + Qt/n + o(t/n)]^n$.  Using the Taylor series for $e^A$, $e^{Qt} = \sum_{k=0}^{\infty}(Qt)^k/k! = lim_{n \to \infty}[I + Qt/n + o(t/n)]^n$.  Because $t$ is fixed, $o(t/n) = o(1/n)$ and
\begin{equation}
  \label{eq:Pt}
  P(t) = e^{Qt}.
\end{equation}
A common approach for solving equation \ref{eq:Pt} is by eigen decomposition.  An $NxN$ matrix, $A$, with $N$ linearly independent eigenvectors can be expressed in its eigen decomposition, $A = B \Lambda B^{-1}$, where $B$ is the $NxN$ matrix whose $i^{th}$ column is the eigenvector $b_i$ of $A$ and $\Lambda$ is the diagonal matrix whose diagonal elements are the corresponding eigenvalues.  Note that $A^2=(B \Lambda B^{-1})(B \Lambda B^{-1}) = B \Lambda (B^{-1}B) \Lambda B^{-1} = B \Lambda^2 B^{-1}$ and in general $A^n = B \Lambda^n B^{-1}$.
The rate matrix can be expressed in its eigen decomposition as
\begin{equation}
  \label{eq:rmed}
  Q = U \Lambda U^{-1}.
\end{equation}
Using the Taylor series for $e^A$ and equation \ref{eq:rmed},
\begin{equation}
  \label{eq:PtEig}
  \begin{split}
...
The solution for the JC model is shown in figure \ref{fig:JC69_Pmat}.
